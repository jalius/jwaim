...
void hack::aim()
{
    ...
    if (!hack::ShouldAimAssist || (hack::aimbotMaxBullets != 0 && shotsFired > hack::aimbotMaxBullets) || (hack::iWeaponID_lp != -1 && helper::IgnoreWeapon(iWeaponID_lp)))
    {
        int AltTwo = 4;
    ...
    if (enemyLifeState != LIFE_ALIVE || idclosestEnt == 0)
    {                        //if the enemy we were aiming at is not alive or is the world...
    ...
    { //if we aren't aiming or are in rage mode...
        ...
        for (int i = 1; i < 64; i++)
        {
            ...
            ;
            if ((shootFriends || ((entitiesCopy[i].entity.m_iTeamNum == 3 && myEnt.m_iTeamNum == 2) || (entitiesCopy[i].entity.m_iTeamNum == 2 && myEnt.m_iTeamNum == 3))) && (entitiesCopy[i].entity.m_bDormant == 0) && (entitiesCopy[i].entity.m_iHealth > 0) && (entitiesCopy[i].entity.ID != myEnt.ID) && (lifeState == LIFE_ALIVE))
            { //if the ent isn't on our team, if they are not dormant, if their health is greater than 0, if they are not us, if they are alive...
                void *m_pStudioBones;
                int closestBone;
                /*float lby;
    ...
    { //if we have an alive target...
        ...
        { //if we have a target and we are still clicking alt2, or if we are in the process of moving to a target...
...
int hack::getClosestBone(unsigned long m_pStudioBonesPtr, std::vector<int> &bones, QAngle &curViewAngle, QAngle &aimPunch, Vector &myPos)
{
    ...
    {
        if (enemyBones[i].x == 0 && enemyBones[i].y == 0 && enemyBones[i].z == 0)
        {             //if we find invalid data...
            continue; //skip this iteration
        }
...
void hack::init()
{
    ...
    unsigned long found_m_iItemDefinitionIndex = (long)client.find(csgo, "\x55\x48\x89\xe5\x53\x48\x83\xec\x08\x0f\xb7\x9f", "xxxxxxxxxxxx") + 12;
